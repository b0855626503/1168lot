...
### Steps
1. ตรึงกติกาหลักของโดเมนก่อนแตกงานย่อย โดยยึด flow ใน `packages/Gametech/Lotto/src/Services/BetService.php` และ invariant ใน `docs/LOTTO_SYSTEM_HANDOVER_TH.md` ได้แก่ draw ต้องเป็น `open`, ต้องเช็ค permission, bet type ต้องมาจาก `BetType::all()`, ต้องเช็คเลขอั้น, min/max, exposure, แล้วจึงสร้าง ticket/item และอัปเดตยอด sold.
