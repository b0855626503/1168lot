...
class CashbackBubu_bk extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        $latestBi = DB::table('bills')
            ->select('bills.member_code', DB::raw('SUM(bills.credit_bonus)  as bonus_amount'), DB::raw("'" . $startDate . "' as date_approve"))
        ...
        $latestWD = DB::table('withdraws', 'withdraws')
            ->select('withdraws.member_code', DB::raw('SUM(withdraws.amount)  as withdraw_amount'), DB::raw("'" . $startDate . "' as date_approve"))
        ...
        $latestBP = DB::table('bank_payment')
            ->select(DB::raw('MAX(bank_payment.code) as code'), DB::raw('MAX(bank_payment.date_approve) as date_approve'), DB::raw('SUM(bank_payment.value) as deposit_amount'), DB::raw("'" . $startDate . "' as date_cashback"), 'bank_payment.member_topup')
