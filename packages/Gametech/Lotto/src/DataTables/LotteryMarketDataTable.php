...
class LotteryMarketDataTable extends DataTable
{
    ...
    {
        $sourceCountSub = DB::table('lotto_result_sources')
            ->selectRaw('market_id, COUNT(*) as source_count')
            ->groupBy('market_id');

        $query = $model->newQuery()
            ->with('group')
            ->leftJoinSub($sourceCountSub, 'source_counts', function ($join): void {
                $join->on('source_counts.market_id', '=', 'lotto_markets.id');
            })
            ->select('lotto_markets.*')
            ->selectRaw('COALESCE(source_counts.source_count, 0) as auto_result_sources_count');
    ...
    protected function getColumns(): array
    {
        return [
            ...
            ['data' => 'draw_mode',  'name' => 'draw_mode',   'title' => 'รูปแบบออกงวด',  'orderable' => false, 'searchable' => false, 'className' => 'text-center'],
            ...
            ['data' => 'result_apply_mode', 'name' => 'auto_settle_on_result', 'title' => 'คำนวน', 'orderable' => false, 'searchable' => false, 'className' => 'text-center', 'width' => '110px'],
            ['data' => 'auto_refund_mode', 'name' => 'auto_refund_on_no_result', 'title' => 'คืนเงิน', 'orderable' => false, 'searchable' => false, 'className' => 'text-center', 'width' => '110px'],
