...
{
    public function build(int $marketId, int $drawId): array
    {
        ...
        $settings = DB::table('lotto_draw_bet_settings')
        ...
        $betStats = DB::table('lotto_ticket_items')
            ...
            ->get([
                ...
                DB::raw(sprintf(
                    ...
                    $hasDiscountAmountAtTime ? 'lotto_ticket_items.discount_amount_at_time' : '0'
                )),
                DB::raw(sprintf(
                    'COALESCE(SUM(%s), 0) as total_net_amount',
                    $hasPayableAmountAtTime ? 'lotto_ticket_items.payable_amount_at_time' : 'lotto_ticket_items.amount'
                )),
                DB::raw('COALESCE(SUM(lotto_ticket_items.win_amount), 0) as total_win_amount'),
            ])
            ->keyBy('bet_type');

        $exposureRows = DB::table('lotto_number_exposures')
            ->where('draw_id', $drawId)
        ...
        $columns = collect(BetType::all())
            ...
            ->map(function (string $betType) use ($settings, $betStats, $exposureByType): array {
                ...
                $maxSoldAmount = empty($exposures) ? 0.0 : max($exposures);
                ...
                return [
                    ...
                    'max_per_number' => (float) ($setting->max_per_number ?? 0),
                    'payout' => (float) ($setting->payout ?? 0),
                    'discount_percent' => (float) ($setting->discount_percent ?? 0),
                    'total_bet_amount' => (float) ($stats->total_bet_amount ?? 0),
                    'total_discount_amount' => (float) ($stats->total_discount_amount ?? 0),
                    'total_net_amount' => (float) ($stats->total_net_amount ?? 0),
                    'total_win_amount' => (float) ($stats->total_win_amount ?? 0),
                    'max_exposure_amount' => $maxSoldAmount,
    ...
    private function buildSummaryRows(Collection $columns): array
    {
        $rows = [
            ...
            'total_net_amount' => 'รับสุทธิ',
            'total_win_amount' => 'ยอดถูก',
            'max_per_number' => 'ยอดสูงสุดต่อเลข',
        ];
