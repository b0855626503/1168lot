                ...
                <b-col>
                    <b-form-group
                        ...
                        <b-form-input
                                ...
                                type="datetime-local"
                                size="sm"
                                :disabled="!formrefill.id"
                                required
                        ></b-form-input>

                        <small class="text-muted" v-if="!formrefill.id">
                            โปรดค้นหา User ก่อน แล้วจึงระบุวันและเวลาโอน
                        </small>
                    </b-form-group>
                </b-col>
            </b-form-row>

            {{-- ✅ เพิ่ม select โปรโมชั่นใต้ account_code --}}
            <b-form-row>
                <b-col>
                    <b-form-group
                            id="input-group-promotion"
                            label="โปรโมชั่น:"
                            label-for="promotion_id"
                            description="เลือกโปรโมชั่น (ระบบจะไปผูก/ยกเลิกใน member_select_pro ให้เอง)">
                        <b-form-select
                                id="promotion_id"
                                v-model="formrefill.promotion_id"
                                :options="promotions"
                                size="sm"
                                :disabled="promotionLoading || !formrefill.id"
                                @change="onPromotionChanged"
                        ></b-form-select>

                        <small class="text-muted" v-if="!formrefill.id">
                            โปรดค้นหา User ก่อน จึงจะเลือกโปรโมชั่นได้
                        </small>
                    </b-form-group>
                </b-col>
            </b-form-row>
        ...
        window.app = new Vue({
            ...
            data() {
                return {
                    ...
                    promotions: [{value: '', text: '== โปรโมชั่น =='}],
                    promotionLoading: false,

                    _promoInternalChange: false,
                    _lastPromotionId: ''
                };
            },
            created() {
            ...
            methods: {
                ...
                // ✅ เปิด modal refill และ set bank_time ค่า default เป็น "ตอนนี้"
                refill() {
                    this.code = null;
                    this.formrefill = {
                        id: '',
                ...
                // ✅ helper: คืนค่า format สำหรับ input type="datetime-local"
                nowDatetimeLocal() {
                    const d = new Date();
                    const pad = (n) => String(n).padStart(2, '0');
                    // datetime-local ต้องรูปแบบ YYYY-MM-DDTHH:mm
                ...
                async loadPromotions() {
                    ...
                    try {
                        ...
                        console.log(e);
                        this.promotions = [{value: '', text: '== โปรโมชั่น =='}];
                    } finally {
                        this.promotionLoading = false;
                    }
                },

                async onPromotionChanged(newVal) {
                    if (this._promoInternalChange) return;

                    const memberId = this.formrefill.id;
                    if (!memberId) {
                        this._setPromotionSilently('');
                        this._lastPromotionId = '';
                        return;
                    }

                    const nextPromotionId = (newVal ?? '').toString();
                    const prevPromotionId = (this._lastPromotionId ?? '').toString();
                    ...
                    const ok = await this.selectPromotion(memberId, nextPromotionId);
                    ...
                    this._lastPromotionId = nextPromotionId;
                },

                async selectPromotion(memberId, promotionId) {
                    try {
                ...
                async deselectPromotion(memberId) {
                    try {
                        await axios.post("{{ route('admin.promotion.deselectpromotion') }}", {
                            id: memberId
                        });
                    } catch (e) {
                        console.log(e);
                ...
                _setPromotionSilently(val) {
                    this._promoInternalChange = true;
                    this.formrefill.promotion_id = val;
                    this.$nextTick(() => {
                        this._promoInternalChange = false;
                    });
                },
                ...
                async loadUserRefill(event) {
                    ...
                    this.formrefill = Object.assign({}, this.formrefill, {
                        name: response.data.data.name,
                        id: response.data.data.code,
                        // ✅ ถ้ายังไม่มี bank_time ให้ใส่ค่า default
                        bank_time: this.formrefill.bank_time || this.nowDatetimeLocal()
                    });
                ...
                refillSubmit(event) {
                    ...
                    // promotion_id ไม่ส่งตาม requirement
                    const payload = Object.assign({}, this.formrefill);
                    delete payload.promotion_id;

                    // ✅ bank_time จะถูกส่งไปกับ payload
                    this.$http.post("{{ url('member/refill') }}", payload)
                        .then(response => {
                            this.$bvModal.msgBoxOk(response.data.message, {
                                title: 'ผลการดำเนินการ',
                                size: 'sm',
