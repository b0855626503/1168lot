...
class BankinOldDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query): DataTableAbstract
    {
        ...
        return $dataTable->setTransformer(new BankinTransformer);
    ...
    public function query(BankPayment $model)
    {
        ...
        if (empty($startdate)) {
            $startdate = now()->subDay()->toDateString() . ' 00:00:00';
        }
        if (empty($enddate)) {
            $enddate = now()->subDay()->toDateString() . ' 23:59:59';
        }

        return $model
            ->active()->income()->waiting()
            ->with('member')
            ...
            ->whereDate('date_create','<',now()->toDateString())
