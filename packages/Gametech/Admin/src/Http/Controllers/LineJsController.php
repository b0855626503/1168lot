...
class LineJsController extends AppBaseController
{
    ...
    {
        // ค้นหาบัญชี SCB และ GSB ในระบบ
        $scbAccounts = $this->bankAccountRepository->getAccount('scb');
        $gsbAccounts = $this->bankAccountRepository->getAccount('gsb');

        $scbAccNos = $scbAccounts->pluck('acc_no')->toArray();
        $gsbAccNos = $gsbAccounts->pluck('acc_no')->toArray();

        $allAccNos = array_merge($scbAccNos, $gsbAccNos);
        $accParam = implode(',', $allAccNos);

        return view($this->_config['view'], [
            'accParam' => $accParam,
            'scbAccounts' => $scbAccounts,
            'gsbAccounts' => $gsbAccounts,
        ]);
    }
