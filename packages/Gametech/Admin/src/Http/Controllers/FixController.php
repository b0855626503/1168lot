...
class FixController extends AppBaseController
{
    ...
    {
        $mode = strtolower(trim((string) config('gametech.cashback.start.mode', 'range')));

        return in_array($mode, ['range', 'daily'], true) ? $mode : 'range';
    }

    private function resolveCashbackStartTarget(): string
    {
