...
//            $who = app('Gametech\Payment\Repositories\BankHengpayRepository')->findOneWhere(['referenceNo' => $message['data']['reference2']]);
            ...
            $detail = 'Lucky QR ' . $message['orderNoNew'];

            $amount = $message['payAmount'];

            $hash = md5($bank_account->code . $datenow . $amount . $detail);

            $uuid = 'LUK'.str_pad($mcode, 7, "0", STR_PAD_LEFT).'X'.date('His');
            $transactionId = $this->check_uuid3($uuid,$mcode);
            ...
            $chk = app('Gametech\Payment\Repositories\BankPaymentRepository')->findOneWhere(['txid' => $transactionId]);
            if(!$chk){
                app('Gametech\Payment\Repositories\BankPaymentRepository')->create($data);
