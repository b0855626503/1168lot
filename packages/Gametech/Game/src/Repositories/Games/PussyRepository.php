...
class PussyRepository extends Repository
{
     ...
     * Keep interface compatible: return array with at least main/success/msg.
     */
    ...
    public function GameCurl($param, $action)
    {
        ...
        try {
            ...
            // เดิมเคย rescue(..., ..., true) ซึ่ง report ให้แล้ว
            // ที่นี่ไม่ report ซ้ำ เพื่อไม่ spam log (คุณจะ report ที่ caller ก็ได้)
        }
        ...
        // No response at all -> hard connection/timeout failure
        if ($response === null) {
    ...
    public function deposit($username, $amount): array
    {
        ...
        if ($score < 1) {
            ...
            // เดิมใช้ now()->timestamp (วินาที) ทำให้ signature บางช่วงอาจซ้ำถ้าระบบยิงถี่
            // แต่คงพฤติกรรมเดิมไว้เพื่อไม่กระทบ provider; ถ้าอยากนิ่งกว่าใช้ microtime ms ได้
            $time = now()->timestamp;
            $sign = strtoupper(md5(strtolower($this->auth.$username.$time.$this->secretkey)));
