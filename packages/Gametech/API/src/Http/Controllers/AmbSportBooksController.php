...
class AmbSportBooksController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        $param = [];
        $session = $request->all();
        if ($session['productId'] != 'AMBSPORTBOOK') {
            $param = [
                'id' => $session['id'],
                'statusCode' => 40003,
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'AMBSPORTBOOK')
                    ->where('response', 'in')
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if ($item['status'] == 'WAITING') {
                        ...
                        $data_sub = GameLogProxy::where('company', 'AMBSPORTBOOK')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
...
//                        ->whereNull('con_4')
                    ...
                    if ($datasub) {
                        ...
                        $datasub_s = GameLogProxy::where('company', 'AMBSPORTBOOK')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'refundsub')
//                            ->where('con_1', $item['id'])
