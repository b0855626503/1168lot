...
@push('styles')
    ...
    <style>
        ...
        .s2-lg .select2-selection--single {
        ...
        .select2-container--gt-dark .select2-search__field::placeholder {
            color: var(--in-muted);
        }

        /* ให้ “รายการ” เป็นตัวเลื่อน ไม่ปล่อยให้ล้นทั้ง dropdown */
        .select2-container--gt-dark .select2-results {
            flex: 1 1 auto;
            min-height: 0;
        }
...
@section('content')
    <div id="main__content" data-bgset="/assets/wm356/images/index-bg.jpg?v=2"
        ...
        <div class="x-index-content-main-container -anon">
            ...
            @include('wallet::customer.marketing.normal_staff')
        ...
        (function waitForJQuery() {
            if (typeof window.jQuery !== 'undefined') {
                ...
                $(function () {
                    ...
                    {{--                // หรือ redirect เช่น window.location.href = '/home';--}}
                    ...
                    {{--    $.ajax({--}}
                    {{--        url: '{{ route('customer.session.register_staff') }}',--}}
                    {{--        method: 'POST',--}}
                    {{--        data: formData,--}}
                    {{--        processData: false,--}}
                    {{--        contentType: false,--}}
                    {{--        headers: {--}}
                    {{--            'X-CSRF-TOKEN': formData.get('_token') // ตอนนี้มีค่าแน่ เพราะเราใส่ @csrf แล้ว--}}
                    {{--        },--}}
                    {{--        success: function (res) {--}}
                    {{--            if (res.success) {--}}
                    {{--                Swal.fire({ icon: 'success', text: res.message, confirmButtonText: '{{ __('app.status.ok') }}' });--}}
                    {{--                $form[0].reset();--}}
                    {{--                $('#bank').val(null).trigger('change');--}}
                    {{--                $('#password1').val(123123);--}}
                    {{--            } else {--}}
                    {{--                Swal.fire({ icon: 'warning', text: res.message, confirmButtonText: '{{ __('app.status.ok') }}' });--}}
                    {{--            }--}}
                    {{--        },--}}
                    {{--        error: function (xhr) {--}}
                    {{--            console.error(xhr);--}}
                    {{--            Swal.fire({ icon: 'error', text: '{{ __('app.status.error') }}', confirmButtonText: '{{ __('app.status.ok') }}' });--}}
                    {{--        },--}}
                    {{--        complete: function () {--}}
                    {{--            $submitBtn.prop('disabled', false);--}}
                    {{--        }--}}
