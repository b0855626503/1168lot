...
class PaymentOutMaxPay implements ShouldQueue
{
...
//        $order = app('Gametech\Payment\Repositories\WithdrawRepository')->find($id);
        ...
        $order_id = 'MWRD-'.str_pad($order->code, 6, '0', STR_PAD_LEFT).'-'.date('YmdHis');
...
//        $transactionId = 'KING-' . str_pad($member->code, 6, '0', STR_PAD_LEFT) . '-' . date('YmdHis');
        ...
        $name = $member->name;
        $callbackUrl = route('api.maxpay.withdraw.callback');
        $phone = $member->tel;
        ...
        $url = config('maxpay.api_url').'/api/v1/payout/create';
        ...
        if ($response['success'] === true) {
            ...
            $order->status = 1;
            $order->status_withdraw = 'A';
            $order->remark_admin = $order->remark_admin.' กำลัง ทำรายการถอนเงินออกจาก MaxPay โอนเข้าบัญชี '.$member->name.' เลขที่บัญชี '.$member->acc_no.' ธนาคาร '.$member->bank->shortcode.' จำนวน '.$amount;
            $order->save();
            $return['complete'] = true;
