...
class PaymentOutOnPay implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $order_id = 'OWRD-'.str_pad($order->code, 6, '0', STR_PAD_LEFT).'-'.date('YmdHis');
        ...
        $callbackUrl = route('api.onpay.withdraw.callback');

        $withdraw_code = config('onpay.pin_withdraw');

        $param = [
            'order_id' => trim($transactionId),
            'amount' => (float) $amount,
            'bank_acc_no' => trim($acc_no),
            'bank_code' => trim($bankName),
            'bank_acc_name' => trim($member->name),
            'withdraw_code' => trim($withdraw_code),
            'callback_url' => trim($callbackUrl),
        ];

        $url = config('onpay.api_url').'/api/v1/withdraw/create';
        ...
        if ($response['success'] === true) {
            ...
            app('Gametech\Member\Repositories\MemberCreditLogRepository')->create([
                ...
                'remark' => 'รายการแจ้งถอนที่ '.$order->code.' / ไอดีที่ถอน : '.$member->user_name.' จำนวนเงิน '.$amount.' ถูกส่งถอนผ่าน Payment OnPay '.$order_id.' [ '.$response['data']['txn_no'].' ]',
                'kind' => 'OTHER',
                'amount' => $amount,
                'amount_balance' => 0,
                'withdraw_limit' => 0,
            ...
            $order->status = 1;
            $order->status_withdraw = 'A';
            $order->remark_admin = $order->remark_admin.' กำลัง ทำรายการถอนเงินออกจาก OnPay โอนเข้าบัญชี '.$member->name.' เลขที่บัญชี '.$member->acc_no.' ธนาคาร '.$member->bank->shortcode.' จำนวน '.$amount;
            $order->save();
            $return['complete'] = true;
