...
class MaxPay
{
    ...
    {
        $parterId = config('maxpay.partner_id') ?? '';
        ...
        $response = Http::timeout(30)
            ->withHeaders([
                'x-api-key' => config('maxpay.api_key')
            ])
        ...
        if (!File::exists(storage_path('logs/maxpay'))) {
            File::makeDirectory(storage_path('logs/maxpay'));
        }
    ...
    public function create_withdraw($url, $data)
    {
        ...
        Log::channel('maxpay_withdraw_create')->info('เริ่มสร้างรายการถอน', [
            'debug' => $debug,
        ]);
    ...
    public function generateSignature(string $partnerId, string $secret, array $body): string
    {
        ...
        // นำ signature ที่ได้ไปใส่ใน x-maxpay-signature
    ...
    public function create_cancel($url, $data)
    {
        ...
        $response = Http::timeout(30)
            ->withHeaders([
                'x-maxpay-partnerid' => $parterId,
                'x-maxpay-signature' => $this->generateSignature($parterId, config('maxpay.secret_key'), $data),
            ])
        ...
        Log::channel('maxpay_cancel_create')->info('เริ่มสร้างรายการยกเลิก', [
            'debug' => $debug,
        ]);
