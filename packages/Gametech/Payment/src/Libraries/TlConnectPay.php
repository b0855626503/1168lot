...
class TlConnectPay
{
    ...
    {
        $parterId = config('tlconnectpay.partner_id') ?? '';
        ...
        $response = Http::timeout(30)
            ->withHeaders([
                'x-api-key' => config('tlconnectpay.api_key')
            ])
        ...
        if (!File::exists(storage_path('logs/tlconnectpay'))) {
            File::makeDirectory(storage_path('logs/tlconnectpay'));
        }
    ...
    public function create_withdraw($url, $data)
    {
        ...
        Log::channel('tlconnectpay_withdraw_create')->info('เริ่มสร้างรายการถอน', [
            'debug' => $debug,
        ]);
    ...
    public function generateSignature(string $partnerId, string $secret, array $body): string
    {
        ...
        // นำ signature ที่ได้ไปใส่ใน x-tlconnectpay-signature
    ...
    public function create_cancel($url, $data)
    {
        ...
        $response = Http::timeout(30)
            ->withHeaders([
                'x-tlconnectpay-partnerid' => $parterId,
                'x-tlconnectpay-signature' => $this->generateSignature($parterId, config('tlconnectpay.secret_key'), $data),
            ])
        ...
        Log::channel('tlconnectpay_cancel_create')->info('เริ่มสร้างรายการยกเลิก', [
            'debug' => $debug,
        ]);
