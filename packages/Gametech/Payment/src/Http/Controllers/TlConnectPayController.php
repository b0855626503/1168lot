...
use Carbon\Carbon;
use Gametech\Auto\Jobs\UpdateBalanceTlConnectPay;
...
class TlConnectPayController extends AppBaseController
{
    protected $_config;
    ...
    public function index($id)
    {
        ...
        return view('topup.box.wellpay_new', compact('data', 'member', 'banks'));
    }
    ...
    public function indextest($id)
    {
        ...
        return view('topup.box.tlconnectpay_new', compact('data', 'member'));
    }

    public function deposit(Request $request)
    {
        $api = new TlConnectPay();
        $request->validate([
        ...
        Log::channel('tlconnectpay_deposit_create')->info('เริ่มสร้างรายการฝาก', [
            'debug' => 'start',
        ]);

        $bank_account = $this->bankAccountRepository->findOneWhere([
            'banks' => 309, 'bank_type' => 1, 'enable' => 'Y', 'status_auto' => 'Y',
        ]);
        ...
        $min_deposit = config('tlconnectpay.min_deposit', 100);
        ...
        $transactionId = 'TDEP-' . str_pad($member->code, 6, '0', STR_PAD_LEFT) . '-' . date('YmdHis');
        $clientId = config('tlconnectpay.client_id');
        $merchantId = config('tlconnectpay.merchant_no');
        ...
        $name = $member->name;
        $callbackUrl = route('api.tlconnectpay.deposit.callback');
        $type = 'QR';
        $timeout = 5;
        ...
        UpdateBalanceTlConnectPay::dispatch()->onQueue('topup');
        ...
        $url = config('tlconnectpay.api_url') . '/api/v1/deposit/create';
        ...
        if ($response['success'] === true) {
            ...
            $return['url'] = route('api.tlconnectpay.index', ['id' => $response['data']['referenceId']]);
    ...
    public function deposit_callback(Request $request)
    {
        ...
        Log::channel('tlconnectpay_deposit_callback')->info('Callback การฝาก', $message);
        ...
        $apiKey = config('tlconnectpay.api_key');
        $secretKey = config('tlconnectpay.secret_key');
...
//        $username = $message['extendParams']['username'] ?? '';
        ...
        if (WebhookHelper::verifyWebhook($message, $apiKey, $secretKey)) {
            ...
            if ($status === 'completed') {
                ...
                $data = [
                    ...
                    'remark_admin' => '',
                    'emp_topup' => 0,
                    'user_create' => 'รอระบบเติมอัตโนมัติ ทำรายการฝากเงินโดย tlconnectpay QR',
                    'create_by' => 'SYSAUTO',
                ];
    ...
    public function withdraw_callback(Request $request)
    {
        ...
        Log::channel('tlconnectpay_withdraw_callback')->info('Callback การฝาก', $message);
...
//			$username = $message['extendParams']['username'] ?? '';
        ...
        if ($status === 'completed') {
            ...
            $data->save();
            broadcast(new RealTimeMessage('tlconnectpay ' . $transactionId . ' โอนเงินให้ลูกค้าแล้ว ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' รายการแจ้งถอนที่ ' . $data->code));
            ...
            if ($response) {
                broadcast(new \App\Events\RealTimeNewMessage(
                    'tlconnectpay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอดให้ลูกค้าแล้ว',
                    [
                        'ui' => 'toast',
...
//                broadcast(new RealTimeMessage('tlconnectpay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอดให้ลูกค้าแล้ว'));
                $data->remark_admin = '[ Ref ID :' . $refId . ' ] โอนไม่สำเร็จ และ ระบบคืนยอดแล้ว';
            } else {
                broadcast(new \App\Events\RealTimeNewMessage(
                    'tlconnectpay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอด ให้ลูกค้าไม่ได้',
                    [
                        'ui' => 'toast',
                        'as' => 'RealTime.Message.All', // จะเปลี่ยนเป็น RealTime.Message.User ก็ได้ (อย่าลืม listen ฝั่ง JS ให้ตรง)
                        'toast' => [
...
//                broadcast(new RealTimeMessage('tlconnectpay Payment โอนไม่สำเร็จ และระบบคืนยอดไม่ได้  ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId));
            ...
            broadcast(new RealTimeMessage('tlconnectpay ' . $transactionId . ' สถานะ ' . ucfirst($status) . ' รายการถอน ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' รายการแจ้งถอนที่ ' . $data->code));
        }
