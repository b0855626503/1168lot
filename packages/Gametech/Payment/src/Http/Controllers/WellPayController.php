...
class WellPayController extends AppBaseController
{
    ...
    public function indextest($id)
    {
        ...
        return view('topup.box.wellpay_new', compact('data', 'member'));
    }
...
//			$username = $message['extendParams']['username'] ?? '';
        ...
        if ($status === 'completed') {
            ...
            broadcast(new RealTimeNewMessage(
                'WellPay ' . $transactionId . ' โอนเงินให้ลูกค้าแล้ว ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' รายการแจ้งถอนที่ ' . $data->code,
                [
                    'ui' => 'toast',
...
//                    'WellPay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอดให้ลูกค้าแล้ว',
//                    [
//                        'ui' => 'toast',
...
//                broadcast(new RealTimeMessage('wellpay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอดให้ลูกค้าแล้ว'));
                ...
                broadcast(new RealTimeNewMessage(
                    'WellPay ยกเลิกรายการแจ้งถอน ของ ID '.$data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอด ให้ลูกค้าไม่ได้',
                    [
                        'ui' => 'toast',
                        'as' => 'RealTime.Message.All',
                        'toast' => [
...
//                broadcast(new \App\Events\RealTimeNewMessage(
//                    'WellPay Payment โอนเงินไม่สำเร็จ ID : ' . $data->member_user . ' จำนวนเงิน ' . $amount . ' Ref ID ' . $refId . ' ระบบคืนยอด ให้ลูกค้าไม่ได้',
//                    [
//                        'ui' => 'toast',
//                        'as' => 'RealTime.Message.All', // จะเปลี่ยนเป็น RealTime.Message.User ก็ได้ (อย่าลืม listen ฝั่ง JS ให้ตรง)
//                        'toast' => [
