...
class WithdrawSeamlessObserver
{
    ...
    private const REALTIME_MESSAGE_FLAG_ENV = 'PAYMENT_WITHDRAW_SEAMLESS_OBSERVER_REALTIME_MESSAGE';
    private const DEFAULT_ENABLE_TELEGRAM_MESSAGE = false;
    private const TELEGRAM_MESSAGE_FLAG_ENV = 'PAYMENT_WITHDRAW_SEAMLESS_OBSERVER_TELEGRAM_MESSAGE';
    ...
    {
        $count = app('Gametech\\Payment\\Repositories\\WithdrawSeamlessRepository')
            ->where('status', 0)
            ->where('enable', 'Y')
            ->count();
...
HTML;
            ...
            TelegramBot::Send('notify/send', $message, $this->telegramNotifyPayload('/withdraw_seamless'));
        } catch (\Throwable $e) {
            report($e);
        }
    }

    private function telegramNotifyPayload(string $path = '/withdraw_seamless'): array
    {
