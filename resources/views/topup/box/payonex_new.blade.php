...
@push('scripts')
    ...
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            ...
            function expireAction() {
                fetch("{{ route('api.payonex.deposit.expire', ['txid' => $data['detail']]) }}", {
                    method: 'POST',
                    headers: {
            ...
            async function handleDownload() {
                ...
                function canvasToBlobSafe(cv) {
                    return new Promise((res, rej) => {
                        if (cv.toBlob) cv.toBlob(b => b ? res(b) : rej(new Error('toBlob failed')), 'image/png');
                            ...
                            try {
                                res(dataURLToBlob(cv.toDataURL('image/png')));
                            } catch (e) {
                                rej(e);
                            }
                        }
                ...
                async function tryMakePosterPngBlob() {
                    try {
                        ...
                        ctx.fillText(lines[0], width / 2, 28);
                ...
                if (posterBlob) {
                    ...
                    if (supportsDownloadAttr() && !isIOS()) {
                        downloadBlob(posterBlob, file.name);
                        return;
                    }
    ...
    <script>
        ...
        const interval = setInterval(async () => {
            try {
                const res = await fetch("{{ route('api.payonex.deposit.status', ['txid' => $data['detail']]) }}");
