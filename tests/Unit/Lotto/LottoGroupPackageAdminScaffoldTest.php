...
class LottoGroupPackageAdminScaffoldTest extends TestCase
{
    ...
    public function test_group_package_admin_keeps_inactive_packages_editable(): void
    {
        $dashboardController = file_get_contents($this->rootPath.'/packages/Gametech/Lotto/src/Http/Controllers/Admin/LottoGroupPackageDashboardController.php');
        $view = file_get_contents($this->rootPath.'/packages/Gametech/Lotto/src/Resources/views/admin/module/lotto/group_packages/index.blade.php');

        $this->assertNotFalse($dashboardController);
        $this->assertNotFalse($view);
        $this->assertStringNotContainsString("->where('is_active', true)", $dashboardController);
        $this->assertStringContainsString('include_inactive: true', $view);
        $this->assertStringContainsString('ปิดใช้งาน', $view);
    }
}
